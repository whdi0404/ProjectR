    ...
    public override void Update(float dt)
    {
        ...
        if (age >= breedableAge)
        {
            ...
            if (breeding >= breedPeriod )
            {
                ...
                if (WorldManager.IsPlantable(MapTilePosition + new Vector2Int(-1, -1)) == true)
                    breedCandidate.Add(MapTilePosition + new Vector2Int(-1, -1));
                if (WorldManager.IsPlantable(MapTilePosition + new Vector2Int(-1, 0)) == true)
                    breedCandidate.Add(MapTilePosition + new Vector2Int(-1, 0));
                if (WorldManager.IsPlantable(MapTilePosition + new Vector2Int(-1, 1)) == true)
                    breedCandidate.Add(MapTilePosition + new Vector2Int(-1, 1));
                if (WorldManager.IsPlantable(MapTilePosition + new Vector2Int(0, -1)) == true)
